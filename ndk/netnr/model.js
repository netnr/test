...
async () => {
    ...
    ndkGenerateCode.echoTableColumn(tableObj => {
        ...
        let classInfo = {
            ...
            className: tableObj.sntn.split('.').pop()
        }
        ...
        // 类注释
        codes.push(`\t/// <summary>`);
        (tableObj.tableColumns[0].TableComment || "").split("\n").forEach(x => codes.push(`\t/// ${x.trim()}`));
        codes.push(`\t/// </summary>`);
        ...
        tableObj.tableColumns.forEach(columnObj => {
            ...
            //类型 二次处理
            if (tableObj.ctype == "Oracle" && lgtype == "decimal" && columnObj.DataScale == 0) {
                lgtype = "int";
            }

            //数据库可为空、类型可为空
            if (columnObj.IsNullable == 1 && typeMap.isNullable.includes(lgtype)) {
                lgtype += "?";
            }
            codes.push(`\t\tpublic ${lgtype} ${columnObj.ColumnName} { get; set; }`);
        });
